...
public class TerminalWriter {
	...
	private SessionLogWriter sessionLogWriter;// = Logger.getLogger(this.toString());
	private ArrayList<ResponseBuffer> responseBuffers=new ArrayList<ResponseBuffer>();

	private static final Logger logger = LoggerFactory.getLogger(TerminalWriter.class.getName());
	...
	public TerminalWriter(final TerminalDisplay term, final TerminalBackBuffer buf, final StyleState styleState) {
        this.charDecoder = Charset.defaultCharset().newDecoder();
		this.display = term;
		this.backBuffer = buf;
	...
	public void writeASCII(final byte[] chosenBuffer, final int start, final int length) {
		...
		try {
			cs=this.charDecoder.decode(ByteBuffer.wrap(chosenBuffer,start,length));
		} catch (CharacterCodingException e) {
		}

		for (ResponseBuffer bfr : responseBuffers) {
			if(bfr.isEnabled()){
				if(cs!=null){
					bfr.append(cs);
				}
		...
		if(cs!=null){
			this.sessionLogWriter.append(cs);
		}
	...
	public void writeString(String string) {
		...
		sessionLogWriter.write(string);
//		if(messageLogger!=null){
//			try {
//				messageLogger.write(string);
//			} catch (IOException e) {
...
//		}
	...
	public void scrollY() {
		...
		try {
			if (cursorY > scrollRegionBottom) {
				...
				scrollArea(scrollRegionTop, scrollRegionBottom - scrollRegionTop, dy);
	...
	public void index() {
		...
		try {
			if (cursorY == termHeight) {
				scrollArea(scrollRegionTop, scrollRegionBottom - scrollRegionTop, -1);
				backBuffer.clearArea(0, scrollRegionBottom - 1, termWidth, scrollRegionBottom);
			} else {
				cursorY += 1;
				display.setCursor(cursorX, cursorY);
			}
	...
	public void reverseIndex() {
		...
		try {
			if (cursorY == 1) {
				scrollArea(scrollRegionTop - 1, scrollRegionBottom - scrollRegionTop, 1);
				backBuffer.clearArea(cursorX, cursorY - 1, termWidth, cursorY);
			} else {
				cursorY -= 1;
				display.setCursor(cursorX, cursorY);
	...
	static Color[] colors = { Color.BLACK, Color.RED, Color.GREEN, Color.YELLOW, Color.BLUE, Color.MAGENTA, Color.CYAN,
			Color.WHITE };
