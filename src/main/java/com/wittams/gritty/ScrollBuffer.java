...
public class ScrollBuffer implements StyledRunConsumer {
	private static final Logger logger=LoggerFactory.getLogger(ScrollBuffer.class.getName());
	...
	public synchronized String getLines() {
		...
		if(completeSections.size()>0)currentLine+=1;
	...
	public synchronized void  consumeRun(final int x, final int y, final Style style, final char[] buf, final int start, final int len) {
		...
		if (bufPos < 0){
			...
			while(totalLines > lineCountLimit)
			{
				Section sec=completeSections.remove(0);
				totalLines-=sec.getLineCount();
				sec=null;
			}
			newSection();
			currentRun++;
