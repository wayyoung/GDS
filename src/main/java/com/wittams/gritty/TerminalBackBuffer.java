...
public class TerminalBackBuffer {
	private static final Logger logger = LoggerFactory.getLogger(TerminalBackBuffer.class.getName());
	...
	public TerminalBackBuffer(final int width, final int height, StyleState styleState) {
	...
	public void clearArea(final int leftX, final int topY, final int rightX, final int bottomY) {
		...
		for (int y = topY; y < bottomY; y++) {
			if (y > height - 1 || y < 0) {
				...
				Arrays.fill(buf, y * width + leftX, y * width + rightX, EMPTY_CHAR);
				Arrays.fill(styleBuf, y * width + leftX, y * width + rightX, Style.EMPTY);
				damage.set(y * width + leftX, y * width + rightX, true);
			}
		}
	}
	...
	public void drawBytes(final byte[] bytes, final int s, final int len, final int x, final int y) {
		...
		if (adjY >= height || adjY < 0) {
			if (logger.isDebugEnabled()) {
				StringBuffer sb = new StringBuffer("Attempt to draw line ").append(adjY).append(" at (").append(x)
	...
	public void pumpRuns(final int x, final int y, final int w, final int h, final StyledRunConsumer consumer) {
		...
		try {
			for (int row = startRow; row < endRow; row++) {
				...
				// end row
				if (lastStyle == null)
					logger.error("Style is null for run supposed to be from " + beginRun + " to " + endCol + "on row "
							+ row);
				else
